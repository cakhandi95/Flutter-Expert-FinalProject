...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    ...
    context.read<MovieDetailBloc>().add(
        OnMovieDetailDataRequested(widget.id)
    );
    context.read<MovieDetailBloc>().add(
        OnMovieRecommendationRequested(widget.id)
    );
  }
    ...
    return Scaffold (
      body: BlocConsumer<MovieDetailBloc,MovieDetailState>(
          builder: (context, state) {
            if (state is MovieDetailIsLoading) {
              return Center(
                                    ...
                                    onPressed: () async {
                                      if (!isAddedWatchlist) {
                                        context.read<WatchListStatusBloc>().add(
                                            OnListWatchAdd(movie)
                                        );
                                      } else {
                                        context.read<WatchListStatusBloc>().add(
                                            OnListWatchRemoved(movie)
                                        );
                                      }
                                    },
                                    child: Row(
